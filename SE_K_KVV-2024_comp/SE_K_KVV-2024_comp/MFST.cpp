...
namespace MFST
{
    ...
    char* Mfst::getDiagnosis(short n, char* buf, Log::LOG log)
    {
        ...
        if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
        {
            ...
            *log.stream << buf;
            sprintf_s(buf, MFST_DIAGN_MAXSIZE, "\033[31mОшибка\033[32m %d \033[0m: %s\n\tЛексема: '\033[31m%c\033[0m', Строка \033[32m%d\033[0m", err.id, err.message, lex.table[lpos].lexema, lex.table[lpos].sn + 1);
            rc = buf;
        }
        return rc;
    }
